...
app.post("/api/hits", postLimiter, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const record = {
      ...
      damage: payload.fightDamage?.damage ?? null,
      minDamage: payload.fightDamage?.min_damage ?? null,
      maxDamage: payload.fightDamage?.max_damage ?? null,
      minDamageWithoutBonus: payload.fightDamage?.min_damage_without_bonus ?? null,
      maxDamageWithoutBonus: payload.fightDamage?.max_damage_without_bonus ?? null,
